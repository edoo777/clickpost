...
export function KanbanColumnsManager({ brandId, stages, onClose }: KanbanColumnsManagerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="flex max-h-[85vh] w-full max-w-xl flex-col gap-4 overflow-y-auto rounded-xl border border-border bg-surface p-5  ">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-foreground ">Gérer les colonnes</h2>
          <button
            type="button"
            onClick={onClose}
            ...
            >
              <div className="flex flex-wrap items-center gap-2">
                <input
                  ...
                  className="min-w-0 flex-1 rounded-lg border border-border bg-surface px-2.5 py-1 text-sm text-zinc-800   dark:text-zinc-200"
                />
                <button
                  type="button"
                  onClick={() => handleMove(stage.id, -1)}
                  disabled={index === 0}
                  className="rounded-lg border border-border px-2 py-1 text-xs text-zinc-600 disabled:cursor-not-allowed disabled:opacity-40  dark:text-zinc-400"
                >
                  ↑
                </button>
                <button
              ...
              <div className="flex flex-wrap items-center gap-2">
                <select
                  ...
                  className="rounded-lg border border-border bg-surface px-2.5 py-1 text-sm text-zinc-700   dark:text-zinc-300"
                >
        ...
        <button
          type="button"
          onClick={handleAdd}
          className="w-fit rounded-lg border border-border px-3 py-1.5 text-sm font-medium text-zinc-600 transition-colors hover:border-violet-200 hover:bg-violet-50 hover:text-violet-700  dark:text-zinc-400 dark:hover:border-violet-500/30 dark:hover:bg-violet-500/10 dark:hover:text-violet-300"
        >
          + Ajouter une colonne
        </button>
      </div>
