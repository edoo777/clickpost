...
export function IdeaWorkshopView({ ideaId }: IdeaWorkshopViewProps) {
  ...
  const primaryAction = (() => {
    if (!idea.publicationId) {
      return {
        label: "Transformer en publication",
        onClick: handleCreatePublication,
        disabled: !idea.platform || isCreatingPublication,
        disabledHint: !idea.platform ? "Sélectionnez un réseau dans les propriétés." : undefined,
      };
    }
    if (idea.status === "in_review") return { label: "Approuver", onClick: () => handleChangeStatus("approved") };
    if (idea.status === "approved") return { label: "Planifier", onClick: () => handleChangeStatus("ready_to_schedule") };
    if (idea.status === "scheduled" || idea.status === "published" || idea.status === "ready_to_schedule") return null;
    return { label: "Envoyer en révision", onClick: () => handleChangeStatus("in_review") };
  })();
          ...
          <div className="mx-auto flex w-full max-w-3xl flex-col rounded-2xl border border-border bg-surface p-6 shadow-sm sm:p-10">
            {mode === "document" ? (
              <WorkshopEditor
                ...
                placeholder="Écrivez votre contenu, ou tapez « / » pour insérer un bloc…"
              />
            ) : (
              ...
              >
                <WorkshopEditor
                  ...
                  onChange={handleDocumentChange}
                  onRewriteSelection={handleRewriteSelection}
                  placeholder="Développez le corps du message…"
                />
              </StructuredModeFields>
            )}
          </div>
