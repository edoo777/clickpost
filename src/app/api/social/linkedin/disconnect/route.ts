...
import { deleteConnection } from "@/lib/linkedin/connections";
...
/**
 * Déconnexion contrôlée : supprime le jeton chiffré (révocation locale — LinkedIn ne propose pas
 * d'endpoint de révocation programmatique dans l'API Community Management standard, la
 * suppression locale empêche tout nouvel appel avec ce jeton depuis ClickPost) et remet le compte
 * à `profile_only` plutôt que de le supprimer entièrement — l'historique des publications déjà
 * envoyées via ce compte reste intact et consultable.
 */
export async function POST(request: Request) {
  ...
  if (row.platform !== "linkedin") return errorResponse("invalid_request", "Ce compte n'est pas un compte LinkedIn.", 400);
