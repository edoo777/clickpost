...
/** Seule route qui a le droit de vérifier `isMetaOAuthConfigured("instagram")` pour l'affichage
 * (dépend de variables d'environnement serveur, jamais disponibles côté client). Ne renvoie
 * jamais de jeton. */
export async function GET(request: Request) {
  ...
  const configured = isMetaOAuthConfigured("instagram");

  if (!accountId) {
    return NextResponse.json({ status: "ok", summary: computeSocialConnectionState(undefined, configured, INSTAGRAM_SCOPES) });
  }
  ...
  return NextResponse.json({ status: "ok", summary: computeSocialConnectionState(account, configured, INSTAGRAM_SCOPES) });
}
