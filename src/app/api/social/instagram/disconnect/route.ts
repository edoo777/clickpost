...
import { deleteConnection } from "@/lib/instagram/connections";
...
/** Déconnexion contrôlée — voir src/app/api/social/linkedin/disconnect/route.ts (implémentation
 * de référence). Remet le compte à `profile_only` plutôt que de le supprimer : l'historique des
 * publications déjà envoyées reste intact. */
export async function POST(request: Request) {
  ...
  if (row.platform !== "instagram") return errorResponse("invalid_request", "Ce compte n'est pas un compte Instagram.", 400);
