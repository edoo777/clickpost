...
import { isXOAuthConfigured, X_SCOPES } from "@/lib/x/config";
...
export async function GET(request: Request) {
  ...
  if (!accountId) {
    return NextResponse.json({ status: "ok", summary: computeSocialConnectionState(undefined, configured, X_SCOPES) });
  }
  ...
  return NextResponse.json({ status: "ok", summary: computeSocialConnectionState(account, configured, X_SCOPES) });
}
