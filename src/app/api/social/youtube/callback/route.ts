...
import { YOUTUBE_SCOPES } from "@/lib/youtube/config";
import { exchangeCodeForToken, fetchYouTubeIdentity, verifyCallbackState } from "@/lib/youtube/oauth";
...
export async function GET(request: Request) {
  ...
  if (errorParam) {
    return NextResponse.redirect(new URL("/comptes?youtube_error=consent_denied", request.url));
  }
  if (!code || !state) {
    return NextResponse.redirect(new URL("/comptes?youtube_error=missing_params", request.url));
  }

  const cached = getCachedCallbackOutcome("youtube", code);
  if (cached) return NextResponse.redirect(new URL(cached, request.url));

  const resultPath = await processCallback(code, state);
  cacheCallbackOutcome("youtube", code, resultPath);
...
async function processCallback(code: string, state: string): Promise<string> {
  const errorPath = (reason: string) => `/comptes?youtube_error=${reason}`;
  ...
  if (!tokenResult.token.refreshToken) {
    // `access_type=offline`+`prompt=consent` garantissent normalement un refresh_token — son
    // absence signale une configuration Google Cloud incorrecte (type de client, écran de
    // consentement) plutôt qu'un cas normal : jamais une connexion acceptée sans pouvoir se
    // renouveler automatiquement, ce qui forcerait une reconnexion toutes les heures.
    return errorPath("missing_refresh_token");
  }

  const identityResult = await fetchYouTubeIdentity(tokenResult.token.accessToken);
  ...
  const missingScopes = YOUTUBE_SCOPES.filter((scope) => !tokenResult.token.scopes.includes(scope));
  ...
  try {
    await saveConnection(workspaceId, accountId, "youtube", tokenResult.token);
  } catch {
    if (!wasExisting) {
  ...
  if (status === "connected") {
    await recordProductEvent(supabase, { eventName: "social_connected", userId: user.id, workspaceId, metadata: { platform: "youtube" } });
  }

  return `/comptes?youtube_connected=${accountId}`;
}
