...
import { isYouTubeOAuthConfigured, YOUTUBE_SCOPES } from "@/lib/youtube/config";
...
export async function GET(request: Request) {
  ...
  if (!accountId) {
    return NextResponse.json({ status: "ok", summary: computeSocialConnectionState(undefined, configured, YOUTUBE_SCOPES) });
  }
  ...
  return NextResponse.json({ status: "ok", summary: computeSocialConnectionState(account, configured, YOUTUBE_SCOPES) });
}
