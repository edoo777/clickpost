...
import { isTikTokOAuthConfigured, TIKTOK_SCOPES } from "@/lib/tiktok/config";
...
export async function GET(request: Request) {
  ...
  if (!accountId) {
    return NextResponse.json({ status: "ok", summary: computeSocialConnectionState(undefined, configured, TIKTOK_SCOPES) });
  }
  ...
  return NextResponse.json({ status: "ok", summary: computeSocialConnectionState(account, configured, TIKTOK_SCOPES) });
}
