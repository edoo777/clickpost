...
export async function GET(request: Request) {
  ...
  if (!isTikTokOAuthConfigured()) {
    return NextResponse.redirect(new URL("/comptes?tiktok_error=not_configured", request.url));
  }

  const { searchParams } = new URL(request.url);
  const brandId = searchParams.get("brandId");
  if (!brandId) {
    return NextResponse.redirect(new URL("/comptes?tiktok_error=missing_brand", request.url));
  }

  const { data: brand, error: brandError } = await supabase.from("brands").select("id, workspace_id").eq("id", brandId).single();
  if (brandError || !brand) {
    return NextResponse.redirect(new URL("/comptes?tiktok_error=brand_not_found", request.url));
  }
  ...
  if (!admin) {
    return NextResponse.redirect(new URL("/comptes?tiktok_error=forbidden", request.url));
  }

  const authorizationUrl = buildAuthorizationUrl({ workspaceId, brandId, userId: user.id, platform: "tiktok" });
