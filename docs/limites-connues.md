...
- **Génération de contenu par IA** (Atelier, Assistant IA, générateur de sujets) : entièrement simulée côté client (`src/lib/content-generation-provider.ts` et équivalents), aucun appel à un service d'IA réel. L'interface est conçue pour qu'un vrai fournisseur puisse être branché derrière la même interface (`ContentGenerationProvider`) sans changement d'UI.
