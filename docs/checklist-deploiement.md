...
|---|---|---|
| `authenticated_security_definer_function_executable` | `is_workspace_member`, `is_workspace_admin`, `ensure_default_workspace` | **Intentionnel** — appelées via RPC par l'application en tant qu'utilisateur authentifié ; `EXECUTE` déjà révoqué pour `anon`. Ne pas « corriger ». |
